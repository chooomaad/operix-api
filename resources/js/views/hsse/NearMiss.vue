<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <h2 class="text-xl font-bold">{{ t('nav.near_miss') }}</h2>
    ...
    <div class="flex gap-3 flex-wrap">
      ...
      <select v-model="params.severity" @change="fetch()" class="px-3 py-2 rounded-lg border border-gray-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-sm">
        <option value="">Toutes sévérités</option><option v-for="s in ['low','medium','high']" :key="s" :value="s">{{ t('severity.'+s) }}</option>
      </select>
    ...
    <div class="bg-white dark:bg-surface-800 rounded-xl border border-gray-200 dark:border-surface-700 overflow-hidden">
      ...
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-surface-700/50 text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
          <tr><th class="px-4 py-3 text-left">Réf.</th><th class="px-4 py-3 text-left">Date</th><th class="px-4 py-3 text-left">Lieu</th><th class="px-4 py-3 text-left">Sévérité</th><th class="px-4 py-3 text-left">Statut</th><th class="px-4 py-3 text-right">Actions</th></tr>
        </thead>
        <tbody class="divide-y divide-gray-100 dark:divide-surface-700">
          <tr v-if="!items.length"><td colspan="6" class="px-4 py-12 text-center text-gray-400">Aucun presqu'accident</td></tr>
          <tr v-for="nm in items" :key="nm.id" class="hover:bg-gray-50 dark:hover:bg-surface-700/40 transition">
            <td class="px-4 py-3 font-mono text-xs text-gray-500">{{ nm.reference }}</td>
            <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ nm.date }}</td>
            <td class="px-4 py-3 font-medium">{{ nm.location }}</td>
            <td class="px-4 py-3"><Badge :value="nm.severity" :label="t('severity.'+nm.severity)" /></td>
            <td class="px-4 py-3"><Badge :value="nm.status" :label="t('status.'+nm.status)" /></td>
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-1">
                <button @click="openEdit(nm)" class="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-surface-700 text-gray-400 hover:text-blue-600 transition"><Icon name="edit" class="w-4 h-4" /></button>
                <button @click="confirmDelete(nm)" class="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-surface-700 text-gray-400 hover:text-red-600 transition"><Icon name="trash-2" class="w-4 h-4" /></button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <Modal v-model="showModal" :title="editing ? 'Modifier presqu\'accident' : 'Nouveau presqu\'accident'" size="lg">
      <form @submit.prevent="save" class="grid grid-cols-2 gap-4">
        ...
        <Field label="Sévérité *" :error="errors.severity">
          <select v-model="form.severity" class="field" required><option value="">—</option><option v-for="s in ['low','medium','high']" :key="s" :value="s">{{ t('severity.'+s) }}</option></select>
        </Field>
    ...
    <ConfirmDialog v-model="showConfirm" message="Supprimer ce presqu'accident ?" :loading="deleting" @confirm="doDelete" />
  </div>
</template>
<script setup>
...
const { t } = useI18n()
const { items, meta, loading, saving, deleting, errors, params, fetch, create, update, remove, goPage } = useCrud('/near-miss')
const showModal=ref(false), showConfirm=ref(false), editing=ref(null), toDelete=ref(null)
const form = reactive({ date:'', time:'', location:'', severity:'', status:'open', description:'', corrective_action:'' })
let debounce=null
...
function openEdit(n){ Object.assign(form,{ date:n.date, time:n.time||'', location:n.location, severity:n.severity, status:n.status, description:n.description||'', corrective_action:n.corrective_action||'' }); editing.value=n; showModal.value=true }
async function save(){ try{ editing.value ? await update(editing.value.id,form) : await create(form); showModal.value=false }catch{} }
function confirmDelete(n){ toDelete.value=n; showConfirm.value=true }
